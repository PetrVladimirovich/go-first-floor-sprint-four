...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var result float64
	result = (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)/height*100)*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	var result float64
	result = (swimmingMeanSpeed(lenghtPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
